                                ...
                                <tbody class="bg-white divide-y divide-gray-200 divide-solid">
                                    ...
                                    <tr class="bg-white">
                                        ...
                                        <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                                            ...
                                            <x-primary-button trigger:true wire:click="editCategory({{ $category->id }})">
                                                
                                                Edit
                                            </x-primary-button>
                                        
                                            <button wire:click="deleteCategory({{ $category->id }})" class="px-4 py-2 text-xs text-red-500 uppercase bg-red-200 rounded-md border border-transparent hover:text-red-700 hover:bg-red-300">
                                                Delete
                                            </button>
                                            @endif
                                            </div>
            ...
            <form wire:submit.prevent="save" class="w-full">
                <div class="flex flex-col items-start p-4">
                    <div class="flex items-center pb-4 mb-4 w-full border-b">
                        ...
                        <svg wire:click.prevent="$set('showModal', false)" class="ml-auto w-6 h-6 text-gray-700 cursor-pointer fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18 18">
                    ...
                    <div class="mb-2 w-full">
                        ...
                        <input wire:model="category.name" id="category.name" class="py-2 pr-4 pl-2 mt-2 w-full text-sm rounded-lg border border-gray-400 sm:text-base focus:outline-none focus:border-blue-400" />
                        @error('category.name')
                        <span class="text-sm text-red-500">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="mb-2 w-full">
                        ...
                        <input wire:model="category.slug" id="category.slug" class="py-2 pr-4 pl-2 mt-2 w-full text-sm rounded-lg border border-gray-400 sm:text-base focus:outline-none focus:border-blue-400" />
                        @error('category.slug')
                        <span class="text-sm text-red-500">{{ $message }}</span>
                        @enderror
                    </div>
